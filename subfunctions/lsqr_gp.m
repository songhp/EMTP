function [x,flag,relres,iter,resvec,lsvec] = lsqr_gp(A,b,I,tol,maxit,M1,M2,x0,varargin)

% use: [x_I, flag] = lsqr_gp(@lsqr_fun, y, candidate_set, OptTol, maxiter_lsqr);

% LSQR for Greedy Pursuit
% I: active set
...
if isequal(atype,'matrix')
    ...
    x = iterapp(afun,atype,afcnstr,b,I,varargin{:},'transp');
    n = size(x,1);
end
...
%    u = b - iterapp(afun,atype,afcnstr,x,varargin{:});
    u = b - iterapp(afun,atype,afcnstr,x,I,varargin{:},'notransp');
end
beta = norm(u);
...
if isequal(atype,'matrix')
    ...
    v = iterapp(afun,atype,afcnstr,u,I,varargin{:},'transp');
end
if existM1
    if isequal(m1type,'matrix')
        v = M1' \ v;
    else
        v = iterapp(m1fun,m1type,m1fcnstr,v,I,varargin{:},'transp');
    end
    if isinf(norm(v,inf))
        flag = 2;
        relres = normr/n2b;
...
if existM2
    if isequal(m2type,'matrix')
        ...
        v = iterapp(m2fun,m2type,m2fcnstr,v,I,varargin{:},'transp');
    end
    if isinf(norm(v,inf))
        flag = 2;
        relres = normr/n2b;
...
%             z = iterapp(m2fun,m2type,m2fcnstr,v,varargin{:});
            z = iterapp(m2fun,m2type,m2fcnstr,v,I,varargin{:},'notransp');
        end
        if isinf(norm(z,inf))
            flag = 2;
            iter = i-1;
...
%             z = iterapp(m1fun,m1type,m1fcnstr,z,varargin{:});
            z = iterapp(m1fun,m1type,m1fcnstr,z,I,varargin{:},'notransp');
        end
        if isinf(norm(z,inf))
            flag = 2;
            iter = i-1;
...
%         u = iterapp(afun,atype,afcnstr,z,varargin{:}) - alpha * u;
        u = iterapp(afun,atype,afcnstr,z,varargin{:},I,'notransp') - alpha * u;
    end
    beta = norm(u);
    u = u / beta;
    ...
    if isequal(atype,'matrix')
        ...
        vt = iterapp(afun,atype,afcnstr,u,I,varargin{:},'transp');
    end
    if existM1
        if isequal(m1type,'matrix')
            vt = M1' \ vt;
        else
            vt = iterapp(m1fun,m1type,m1fcnstr,vt,I,varargin{:},'transp');
        end
    ...
    if existM2
        if isequal(m2type,'matrix')
            ...
            vt = iterapp(m2fun,m2type,m2fcnstr,vt,I,varargin{:},'transp');
        end
        if isinf(norm(vt,inf))
            flag = 2;
            iter = i;
    ...
    % Thong Do added the following line to prevent the error msg divide by zero
    if alpha <1e-15
        break;
    end
